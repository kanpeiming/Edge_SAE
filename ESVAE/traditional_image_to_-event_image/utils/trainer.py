    ...
    def train(self, source_train_loader, target_train_loader, target_val_loader):
        ...
        for epoch in range(self.args.epoch):
            ...
            # for i, ((source_data, source_labels), (target_data, target_labels)) in enumerate(zip(source_train_loader, target_train_iter)):
                ...
                source_outputs, target_outputs, encoder_tl_loss, feature_tl_loss = self.network(source_data.float(), target_data.float(), self.args.encoder_tl_loss_type,self.args.feature_tl_loss_type)
...
class AlignmentTLTrainer(TLTrainer):
    def __init__(self, args, device, writer, network, optimizer, criterion, scheduler, model_path):
        super(AlignmentTLTrainer, self).__init__(args, device, writer, network, optimizer, criterion, scheduler, model_path)
...
class AlignmentTLTrainerOnImageNet(TLTrainer):
    def __init__(self, args, device, writer, network, optimizer, criterion, scheduler, model_path):
        super(AlignmentTLTrainerOnImageNet, self).__init__(args, device, writer, network, optimizer, criterion, scheduler,
                                                 model_path)
...
class ANN2SNNTrainer(Trainer):
    ...
    def train_ann(self, train_loader, val_loader):
        for epoch in range(self.args.epoch):
            ...
            self.writer.add_scalar(tag="ann_train/lr", scalar_value=self.optimizer.param_groups[0]['lr'], global_step=epoch)
...
class AttentionTrainer(Trainer):
    def __init__(self, args, device, writer, network, optimizer, criterion, scheduler, model_path):
        super(AttentionTrainer, self).__init__(args, device, writer, network, optimizer, criterion, scheduler, model_path)

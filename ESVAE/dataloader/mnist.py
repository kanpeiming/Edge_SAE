...
USER_NAME = 'zhan'
DIR = {
    'MNIST': f'/data/{USER_NAME}/Event_Camera_Datasets/MNIST/MNIST/raw',
    'MNISTDVS': f'/data/{USER_NAME}/Event_Camera_Datasets/MNIST/NMNIST/temporal_effecient_training_1.0_np',
    'MNISTDVS_CATCH': f'/data/{USER_NAME}/Event_Camera_Datasets/MNIST/NMNIST_dst_cache',
}
...
def get_tl_mnist(batch_size, train_set_ratio=1.0, dvs_train_set_ratio=1.0):
    ...
    """
    rgb_trans_train = transforms.Compose([transforms.Resize(32),
                                          transforms.ToTensor(),
    ...
    #                                      transforms.Lambda(lambda x: x.repeat(3,1,1)),
    #                                     ])
    dvs_trans = transforms.Compose([transforms.Resize(32),
    ...
    if train_set_ratio < 1.0:
        ...
        tl_train_data, _ = random_split(tl_train_data, [split, n_train - split],
...
def get_n_mnist(batch_size, T, split_ratio=0.9, train_set_ratio=1.0, size=32, encode_type='TET'):
    if encode_type == "spikingjelly":
        ...
        if os.path.exists(train_set_pth) and os.path.exists(test_set_pth):
            ...
            origin_set = n_mnist.NMNIST(root=DIR['MNIST'], train=True, data_type='frame', frames_number=T,
                                        split_by='number',
                                        transform=trans)
...
class NMNIST(Dataset):
    ...
    def __init__(self, root, train=True, transform=True, target_transform=None):
        ...
        self.train = train
        self.resize = transforms.Resize(size=(32, 32))  # 48 48
...
class NMNIST1(Dataset):
        ...
        for label_idx, class_dir in enumerate(class_dirs):
            ...
            if os.path.isdir(class_path):
                ...
                for filename in os.listdir(class_path):
                    if filename.endswith('_np.pt'):  # MNIST数据集使用 _np.pt 后缀
    ...
    def __getitem__(self, index):
        ...
        try:
            loaded_data = torch.load(filepath, weights_only=True)  # 添加 weights_only=True 参数
            if isinstance(loaded_data, tuple):
                data = loaded_data[0]  # 如果是元组，取第一个元素
            else:
                data = loaded_data  # 如果不是元组，直接使用
